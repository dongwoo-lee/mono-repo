              ...
              <b-colxx sm="3">
                <b-form-group label="분류" class="has-float-label">
                  <b-form-select 
                    ...
                    :options="proOptions"
                    :disabled="proOptions.length === 0"
                    value-field="id"
                    text-field="name"
                  >
                    <template v-slot:first>
                      <b-form-select-option v-if="proOptions.length > 0" value="">선택해주세요.</b-form-select-option>
...
export default {
  ...
  created() {
    ...
    this.getProOptions(this.searchItems.media);
    this.getData();
  },
  methods: {
    getData() {
      ...
      this.$http.get(`/api/Products/filler/pr/${brd_dt}`, { params: this.searchItems })
        .then(res => {
