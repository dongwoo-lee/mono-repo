...
export default {
  ...
  methods: {
    ...
    getMenuGrpOptions() {
      this.$http.get("/api/menugrp").then((res) => {
        if (res.status === 200) {
    ...
    setMenuGrpDescription(menu) {
      this.$http.get("/api/menugrp/" + menu.code).then((res) => {
        if (res.status === 200) {
    ...
    getDiskScopeOptions() {
      this.$http.get("/api/disk-scope").then((res) => {
        if (res.status === 200) {
    ...
    onChangeDiskMax(value, item, index) {
      const findOptionItem = this.diskScopeOptions.filter(
    ...
    onChangeMenuGrp(value, item, index) {
      const findOptionItem = this.menuGrpOptions.filter(
    ...
    onUpdate(item, index) {
      ...
      this.$http.put("/api/users", params).then((res) => {
        const { status, data } = res;
        if (status === 200 && !data.errorMsg) {
          this.$fn.notify("primary", {
            message: `${item.name} 사용자 정보 업데이트 성공`,
          });
        } else {
          this.$fn.notify("server-error", {
            message: "조회 에러:" + data.errorMsg,
          });
        }
      });
      ...
      // click trigger
      const rowElem =
        this.$refs.refUserListTable.$el.querySelectorAll("tbody tr")[index];
      rowElem.click();
    },
  },
};
