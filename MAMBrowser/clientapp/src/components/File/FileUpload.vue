<template>
  <b-container class="my-container">
    ...
    <div class="drop-active" v-show="localFiles.length === 0">
      <h4>
        업로드할 파일을 여기에 드래그 하여 올려 놓거나 <br />선택 버튼을 눌러
        탐색기에서 선택하세요.
      </h4>
      <label
        for="file"
        class="btn btn-outline-primary default cutom-label w-50 t-50"
        >파일 선택</label
      >
      <div>
        라디오 방송 업무를 위한 음원 소재
        <span style="font-weight:700;">파일(WAV,MP3)</span>만 업로드 할 수
        있으며, <br />개별 파일당 용량은
        <span style="font-weight:700;">최대 2GB</span>를 넘을수 없습니다.
      </div>
    </div>
    <!-- 파일이 존재할 경우, 테이블 -->
    <div v-show="localFiles.length > 0">
      <div class="c-upload-top d-flex flex-row">
        <div class="flex-grow-1">
          총 {{ localFiles.length }}개 파일 중
          {{ getSuccessUploadFileLength() }}개 업로드 완료
        </div>
      ...
      >
        ...
        <template slot="name" scope="props">
          ...
          <div v-show="props.rowData.uploadState === 'save'">
            ※용량에 따라 저장시간이 오래 걸릴수 있습니다.
          </div>
        </template>
        <!-- 상태 -->
...
export default {
  ...
  methods: {
    ...
    getDeleteState(state) {
      if (
        state === FILE_UPLOAD_STATUS.START ||
        state === FILE_UPLOAD_STATUS.STOP
      )
        return "취소";
      if (state === FILE_UPLOAD_STATUS.SUCCESS || FILE_UPLOAD_STATUS.ERROR)
        return "목록제거";
      if (state === FILE_UPLOAD_STATUS.SAVE) return "저장중";
    ...
    uploadStartState() {
      return (
        !this.getUploadTransmitState &&
        this.localFiles.length > this.getSuccessUploadFileLength()
      );
    },
    uploadStopState() {
      return this.getUploadTransmitState && this.getBeingUploaded;
