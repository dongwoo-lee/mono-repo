<template>
  <div>
    <div
      class="drop-active"
      v-show="$refs.refFileUpload && $refs.refFileUpload.dropActive"
    >
      <!-- <h4>음원 파일 업로드</h4>
        <file-upload 
            ref="refFileUpload" 
    ...
    <common-confirm
      ...
      title="유효하지 않은 파일입니다."
      :message="
        `이 파일은 유효한 파일이 아닙니다. <br>확장자가 ${FILE_UPLOAD_EXTENSIONS}인 파일을 업로드해주세요.`
      "
    />
    <common-confirm
      ...
      title="디스크 공간이 부족합니다."
      :message="
        `디스크 용량이 부족합니다. <br>업로드 가능 사이즈는 ${$fn.formatBytes(
          diskAvailable
        )}입니다.`
      "
    />
...
export default {
  ...
  watch: {
    localFiles(files, oldFiles) {
      ...
      // [MY디스크]업로드 가능 용량 체크
      if (
        this.currentPageName === this.availableDiskPage &&
        this.notDiskAvailable(files)
      ) {
