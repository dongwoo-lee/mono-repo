    ...
    LoadDownloadedFile(){
      ...
      httpClient.get(fileRequestUrl, {
        ...
        if(res.status == 200){
          ...
          httpClient.get(waveformUrl+`?fileName=${fileName}`, {
            cancelToken: source.token,
            headers:{
            'X-Csrf-Token': sessionStorage.getItem(ACCESS_TOKEN),
          }}).then(resWaveForm=>{
              ...
              this.isSuccess = true;
                wavesurfer.load(streamingUrl+`?userid=${userId}&fileName=${fileName}`, resWaveForm.data);
                res.data.resultObject.data.groupData.forEach(dt => {
