...
let mixinCommon = {
  ...
  methods: {
    ...
    // 페이지 정보
    getPageInfo() {
      const dataLength = this.responseData.data
        ? this.responseData.data.length
        : 0;
    ...
    onSortable(sortKey) {
      if (!this.sortItems[sortKey]) {
        ...
        this.sortItems[sortKey] = this.$fn.changeSortValue(
          this.sortItems,
          sortKey
        );
      }
      ...
      console.debug(
        "sorkKey",
        this.searchItems.sortKey,
        this.searchItems.sortValue
      );
      this.getData();
    },
    // 카테고리 API 요청
    ...
    getPgmOptions(brd_dt, media) {
      ...
      this.requestCall(
        `/api/Categories/pgmcodes?brd_dt=${brd_dt}&media=${media}`,
        "pgmOptions"
      );
    },
    // 사용처 분류 선택
    ...
    onMyDiskCopyFromPublic() {
      ...
      this.onMyDisCopy(
        `/api/products/workspace/public/public-to-myspace/${item.seq}`,
        item.title,
        metaData
      );
    },
    onMyDiskCopyFromMusic(item, metaData) {
      ...
      var metaData = this.$refs.refCopyToMySpacePopup.getMetaData();
      this.onMyDisCopy(
        `/api/musicsystem/music-to-myspace?token=${item.fileToken}`,
        item.name,
        metaData
      );
    },
    ...
    onMyDiskCopyFromDl30(item, metaData) {
      ...
      var metaData = this.$refs.refCopyToMySpacePopup.getMetaData();
      this.onMyDisCopy(
        `/api/products/dl30-to-myspace/${item.seq}`,
        item.recName,
        metaData
      );
    },
