...
export default {
  ...
  getters: {
    ...
    getBeingUploaded: state =>
      state.fileData.length > 0 &&
      state.fileData.some(
        file => file.uploadState !== FILE_UPLOAD_STATUS.SUCCESS
      )
  },
  state: {
    fileData: [],
  ...
  mutations: {
    SET_FILES: (state, { files, meta }) => {
      ...
      if (existMetaList.length > 0) {
        ...
        $fn.notify("warning", {
          message: `${metaTitles.join(",")}인 파일이 존재합니다.`
        });
        return;
      }
  ...
  actions: {
          ...
          if (!uploadCancelToken[data.file.id]) {
            ...
            const config = {
              onUploadProgress: progressEvent => {
                ...
                data.file.progress = Math.round(
                  (progressEvent.loaded / data.file.size) * 100
                );
            ...
            try {
              const res = await $http.post(
                `/api/products/workspace/${state.uploadViewType}/files/${userId}`,
                formData,
                config
              );
    ...
    downloadWorkspace({ state }, { ids, type }) {
      //private, public 파일 다운로드
