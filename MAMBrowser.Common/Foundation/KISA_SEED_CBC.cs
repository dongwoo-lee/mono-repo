...
public class KISA_SEED_CBC
{
	...
	private static byte GetB0(long A) { return (byte)(A & 0x0ff); }
	private static byte GetB1(long A) { return (byte)((A >> 8) & 0x0ff); }
	private static byte GetB2(long A) { return (byte)((A >> 16) & 0x0ff); }
	private static byte GetB3(long A) { return (byte)((A >> 24) & 0x0ff); }
	...
	private static void SeedRound(long[] T, long[] LR, int L0, int L1, int R0, int R1, long[] K, int K_offset)
	{
	...
	private static long EndianChange(long dwS)
	{
	...
	private const long KC0 = 0x9e3779b9;
	private const long KC1 = 0x3c6ef373;
	private const long KC2 = 0x78dde6e6;
	private const long KC3 = 0xf1bbcdcc;
	private const long KC4 = 0xe3779b99;
	private const long KC5 = 0xc6ef3733;
	private const long KC6 = 0x8dde6e67;
	private const long KC7 = 0x1bbcdccf;
	private const long KC8 = 0x3779b99e;
	private const long KC9 = 0x6ef3733c;
	private const long KC10 = 0xdde6e678;
	private const long KC11 = 0xbbcdccf1;
	private const long KC12 = 0x779b99e3;
	private const long KC13 = 0xef3733c6;
	private const long KC14 = 0xde6e678d;
	private const long KC15 = 0xbcdccf1b;
	...
	private static void RoundKeyUpdate0(long[] T, long[] K, int K_offset, long[] ABCD, long KC)
	{
	...
	private static void RoundKeyUpdate1(long[] T, long[] K, int K_offset, long[] ABCD, long KC)
	{
		T[0] = ABCD[ABCD_A] + ABCD[ABCD_C] - KC;
		T[1] = ABCD[ABCD_B] + KC - ABCD[ABCD_D];
		K[K_offset + 0] = SS0[GetB0(T[0]) & 0x0ff] ^ SS1[GetB1(T[0]) & 0x0ff] ^ SS2[GetB2(T[0]) & 0x0ff] ^ SS3[GetB3(T[0]) & 0x0ff];
	...
	private static void BLOCK_XOR_CBC(long[] OUT_VALUE, int out_value_offset, long[] IN_VALUE1, int in_value1_offset, long[] IN_VALUE2, int in_value2_offset)
	{
	...
	private static void KISA_SEED_Encrypt_Block_forCBC(long[] inArray, int in_offset, long[] outArray, int out_offset, KISA_SEED_KEY ks)
	{
		long[] LR = new long[4];      // Iuput/output values at each rounds
		long[] T = new long[2];       // Temporary variables for round function F
		long[] K = ks.key_data;      // Pointer of round keys

		// Set up input values for first round
		LR[LR_L0] = inArray[in_offset + 0];
		LR[LR_L1] = inArray[in_offset + 1];
		LR[LR_R0] = inArray[in_offset + 2];
		LR[LR_R1] = inArray[in_offset + 3];
		...
		outArray[out_offset + 0] = LR[LR_R0];
		outArray[out_offset + 1] = LR[LR_R1];
		outArray[out_offset + 2] = LR[LR_L0];
		outArray[out_offset + 3] = LR[LR_L1];

	}

	private static void KISA_SEED_Decrypt_Block_forCBC(long[] inArray, int in_offset, long[] outArray, int out_offset, KISA_SEED_KEY ks)
	{
		long[] LR = new long[4];              // Iuput/output values at each rounds
		long[] T = new long[2];               // Temporary variables for round function F
		long[] K = ks.key_data;              // Pointer of round keys
	...
	public static long[] chartoint32_for_SEED_CBC(byte[] inArray, int inLen)
	{
		long[] data;
		int len, i;
		...
		for (i = 0; i < len; i++)
		{
			Common.byte_to_int(data, i, inArray, i * 4, ENDIAN);
		}
	...
	public static byte[] int32tochar_for_SEED_CBC(long[] inArray, int inLen)
	{
		byte[] data;
		int i;
		...
		else
		{
			for (i = 0; i < inLen; i++)
			{
				data[i] = (byte)(inArray[i / 4] >> ((3 - (i % 4)) * 8));
			}
		}
	...
	{
		long[] ABCD = new long[4];            // Iuput/output values at each rounds(각 라운드 입/출력)
		long[] T = new long[2];               // Temporary variable
		long[] K;
	...
	public static int SEED_CBC_Process(KISA_SEED_INFO pInfo, long[] inArray, int inLen, long[] outArray, int[] outLen)
	{
		...
		if (KISA_ENC_DEC._KISA_ENCRYPT == pInfo.encrypt)
		{
			...
			{
				BLOCK_XOR_CBC(outArray, out_offset, inArray, in_offset, pdwXOR, pdwXOR_offset);

				KISA_SEED_Encrypt_Block_forCBC(outArray, out_offset, outArray, out_offset, pInfo.seed_key);
			...
			Common.memcpy(pInfo.ivec, Array.ConvertAll(pdwXOR, item => (int)item), pdwXOR_offset, BLOCK_SIZE_SEED);
			Common.memcpy(pInfo.cbc_buffer, Array.ConvertAll(inArray, item => (int)item), in_offset, pInfo.buffer_length);
		}
		else
		{
			pdwXOR = pInfo.ivec;
			...
			while (nCurrentCount <= inLen)
			{
				KISA_SEED_Decrypt_Block_forCBC(inArray, in_offset, outArray, out_offset, pInfo.seed_key);

				BLOCK_XOR_CBC(outArray, out_offset, outArray, out_offset, pdwXOR, pdwXOR_offset);
			...
			Common.memcpy(pInfo.ivec, Array.ConvertAll(pdwXOR, item => (int)item), pdwXOR_offset, BLOCK_SIZE_SEED);
			Common.memcpy(pInfo.cbc_last_block, Array.ConvertAll(outArray, item => (int)item), out_offset - BLOCK_SIZE_SEED_INT, BLOCK_SIZE_SEED);
	...
	public static int SEED_CBC_Close(KISA_SEED_INFO pInfo, long[] outArray, int out_offset, int[] outLen)
	{
		int nPaddngLeng;
		int i;
		...
		if (KISA_ENC_DEC._KISA_ENCRYPT == pInfo.encrypt)
		{
			...
			KISA_SEED_Encrypt_Block_forCBC(pInfo.cbc_buffer, 0, outArray, out_offset, pInfo.seed_key);
		...
		else
		{
			...
			if (nPaddngLeng > 0 && nPaddngLeng <= BLOCK_SIZE_SEED)
			{
				...
				{
					Common.set_byte_for_int(outArray, out_offset - i, (byte)0x00, ENDIAN);
				}
	...
	public static byte[] SEED_CBC_Encrypt(byte[] pbszUserKey, byte[] pbszIV, byte[] message, int message_offset, int message_length)
	{
		...
		Array.Copy(message, message_offset, pbszPlainText, 0, message_length);
	...
	public static byte[] SEED_CBC_Decrypt(byte[] pbszUserKey, byte[] pbszIV, byte[] message, int message_offset, int message_length)
	{
		...
		Array.Copy(message, message_offset, pbszCipherText, 0, message_length);
		...
		if (SEED_CBC_Close(info, outbuf, (nRetOutLeng[0]), nPaddingLeng) == 1)
		{
			...
			Array.Copy(pbszPlainText, 0, result, 0, pdmessage_length);
	...
	{
		public long[] key_data = new long[32];
	...
	{
		public int encrypt;
		public long[] ivec = new long[4];
		public KISA_SEED_KEY seed_key = new KISA_SEED_KEY();
		public long[] cbc_buffer = new long[4];
		public int buffer_length;
		public long[] cbc_last_block = new long[4];
	...
	public static class Common
	{
		...
		public static void memcpy(long[] dst, byte[] src, int length, int ENDIAN)
		{
			int iLen = length / 4;
		...
		public static void memcpy(long[] dst, int[] src, int src_offset, int length)
		{
		...
		public static void set_byte_for_int(long[] dst, int b_offset, byte value, int ENDIAN)
		{
		...
		public static byte get_byte_for_int(long[] src, int b_offset, int ENDIAN)
		{
			if (ENDIAN == BIG_ENDIAN)
			{
				int shift_value = (3 - b_offset % 4) * 8;
				int mask_value = 0x0ff << shift_value;
				var value = (src[b_offset / 4] & mask_value) >> shift_value;
				return (byte)value;
			}
			else
			{
		...
		public static void byte_to_int(long[] dst, int dst_offset, byte[] src, int src_offset, int ENDIAN)
		{
			if (ENDIAN == BIG_ENDIAN)
			{
		...
		public static long URShift(int x, int n)
		{
			if (n == 0)
				return x;
			if (n >= 32)
				return 0;
			int v = x >> n;
			long v_mask = ~(0x80000000 >> (n - 1));
			return v & v_mask;
		}

		public static long INT_RANGE_MAX = (long)Math.Pow(2, 32);
